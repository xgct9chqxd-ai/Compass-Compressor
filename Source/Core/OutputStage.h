...
struct OutputStage
{
    ...
    void process (juce::AudioBuffer<float>& buffer)
    {
        ...
        for (int ch = 0; ch < numCh; ++ch)
        {
            ...
            double px2 = x2[(size_t)ch];
            double py2 = y2[(size_t)ch];
            const double a = dcA;
            ...
            for (int i = 0; i < nSamp; ++i)
            {
                ...
                // Post-clip DC block (sealed <= 10 Hz) — removes DC/sub-DC generated by nonlinearity
                {
                    const double xPost = (double) out;
                    const double yPost = (xPost - px2) + a * py2;
                    px2 = xPost;
                    py2 = yPost;
